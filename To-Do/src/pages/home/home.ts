...
export class HomePage {
  ...
  db:any;
  constructor(public navCtrl: NavController, public actionSheetCtrl: ActionSheetController, public taskService: TaskService, public alertCtrl: AlertController, public userService: UserService, private oneSignal: OneSignal, public platform: Platform, public loadingCtrl: LoadingController, private sqliteservice: SQLiteService) {
  ...
  qFilterTasks(val) {
    ...
    // } 
      ...
      this.allTasks = this.taskBackup;
      var delayed = [];
      for (var m = 0; m < this.allTasks.length; m++) {
        if (this.allTasks[m].status == "Delayed") {
          delayed.push(this.allTasks[m])
        }
      }
      ...
      this.allTasks = this.taskBackup;
      var completed = [];
      for (var x = 0; x < this.allTasks.length; x++) {
        if (this.allTasks[x].status == "Completed") {
          completed.push(this.allTasks[x])
        }
      }
    ...
    alert.addButton({
      ...
      handler: data => {
        if (data == "past") {
          ...
          var today1 = new Date(date1);
          today1.setHours(0, 0, 0, 0);
          for (var j = 0; j < this.allTasks.length; j++) {
            var due1 = moment(moment(this.allTasks[j].due_date).format(), moment.ISO_8601).format();
            var formatted_due1 = new Date(due1);
            formatted_due1.setHours(0, 0, 0, 0);
            if (today1 == formatted_due1) {
              formattedDays1.push(this.allTasks[j]);
            }
          }
          this.allTasks = formattedDays1;
          if (this.allTasks.length == 0) {
            this.showMessage = true;
          }else {
            this.showMessage = false;
          ...
          var today2 = new Date(date2);
          today2.setHours(0, 0, 0, 0);
          for (var k = 0; k < this.allTasks.length; k++) {
            var due2 = moment(moment(this.allTasks[k].due_date).format(), moment.ISO_8601).format();
            var formatted_due2 = new Date(due2);
            formatted_due2.setHours(0, 0, 0, 0);
            if (today2 < formatted_due2) {
              formattedDays2.push(this.allTasks[k]);
            }
          }
          this.allTasks = formattedDays2;
          if (this.allTasks.length == 0) {
            this.showMessage = true;
          }else {
            this.showMessage = false;
