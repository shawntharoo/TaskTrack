...
@Component({
  selector: 'page-tasksToMe',
  templateUrl: 'tasksToMe.html',
  animations: [
    trigger('fadeInOut', [
...
export class TasksToMePage {
  ...
  initializeItems() {
    ...
    this.groups = [];
    this.taskService.taskAssignedToUser(this.user).then((res) => {
      this.allTasks = res;
      this.taskBackup = res;
      this.assignedByOthers = [];
      ...
      for(var i=0; i<this.allTasks.length; i++){
        ...
        this.allTasks[i].color ='#4A4C4F';
          if(this.allTasks[i].assigned_by == window.localStorage.getItem("todos_phone_number")){
            ...
            this.assignedByOthers.push(this.allTasks[i].assigned_by)
          }
      }
    }
      ...
      buttons: [
        ...
        },
        {
          ...
          handler: () => {
            ...
            var arr = this.removeDuplicates(this.assignedByOthers, 'assigned_by');
            var newArr = [];
            for(var j=0; j< arr.length; j++){
              newArr.push(this.allTasks.find(o => o.assigned_by === arr[j]))
            }
            ...
            for (var k = 0; k < newArr.length; k++) {
              filterAlert.addInput({
                type: 'radio',
                label: newArr[k].firstname,
                value: newArr[k].assigned_by
              })
            }
            filterAlert.addButton({
              text: 'Ok',
